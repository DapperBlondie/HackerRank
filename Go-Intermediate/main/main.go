...
func postHandler(w http.ResponseWriter, req *http.Request) {
	if req.Method != "POST" {
		http.Error(w, "POST available", http.StatusMethodNotAllowed)
		return
	}
	...
	if err != nil {
		http.Error(w, "BODY is not available", http.StatusBadRequest)
		return
	}
...
func deleteHandler(w http.ResponseWriter, req *http.Request) {
	if req.Method != "DELETE" {
		http.Error(w, "DELETE available", http.StatusMethodNotAllowed)
		return
	}
...
func getHandler(w http.ResponseWriter, req *http.Request) {
	if req.Method != "GET" {
		http.Error(w, "GET available", http.StatusMethodNotAllowed)
		return
	}
	...
	value, ok := store[payload.Payload]
	if ok {
		w.WriteHeader(200)
